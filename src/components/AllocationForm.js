            ...
            <div className='row'>
                <div className='input-group mb-3' style={{ marginLeft: '2rem' }}>
                    ...
                    <select className='custom-select' id='inputGroupSelect01' onChange={(event => setAction(event.target.value))}>
                        <option defaultValue value='Add' namme='Add'>Add</option>
                        <option value='Reduce' name='Reduce'>Reduce</option>
                    </select>
                    <input required='required' type='number' id='cost' value={cost} style={{ marginLeft: '2rem'}} onChange={(event) => setCost(event.target.value)}></input>
